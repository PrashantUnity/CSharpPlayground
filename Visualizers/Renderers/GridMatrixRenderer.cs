...
public class GridMatrixRenderer : VisualizerRendererBase
{
    ...
    public override void Render(DrawingContext context, Rect bounds, VisualizerOptions options)
    {
        ...
        double labelWidth = RowLabelWidth(grid, options);

        double totalGridWidth = labelWidth + grid.Columns * (cellSize + gap);
        ...
        if (coordSize > 0)
        {
            for (int c = 0; c < grid.Columns; c++)
            {
                ...
                double cx = startX + labelWidth + c * (cellSize + gap) + cellSize / 2.0;
        ...
        for (int r = 0; r < grid.Rows; r++)
        {
            if (coordSize > 0)
            {
                ...
                // Right-aligned against the grid, so a label like "answer" never slides under the first cell.
                context.DrawText(ft, new Point(startX + labelWidth - 6 - ft.Width, ry - ft.Height / 2.0));
            }

            for (int c = 0; c < grid.Columns; c++)
            {
                var cell = grid[r, c];
                double x = startX + labelWidth + c * (cellSize + gap);
                ...
                foreach (var arrow in cell.Arrows)
                {
                    ...
                    {
                        double targetX = startX + labelWidth + arrow.TargetCol * (cellSize + gap) + cellSize / 2.0;
    ...
    public override VisualizerHitTestResult? HitTest(Point pointerPosition, Rect bounds, VisualizerOptions options)
    {
        ...
        double relX = pointerPosition.X - (startX + labelWidth);
    ...
    private static double ComputeCellSize(Rect bounds, GridMatrixData grid, VisualizerOptions options)
    {
        ...
        double coordSize = ShowCoordinates(grid, options) ? CoordHeaderSize : 0;
        double availWidth = Math.Max(40, bounds.Width - RowLabelWidth(grid, options) - 32);
    ...
    // Row labels can be words ("prefix", "answer"), wider than the index numbers the header column was sized for.
    private static double RowLabelWidth(GridMatrixData grid, VisualizerOptions options)
    {
        if (!ShowCoordinates(grid, options)) return 0;
        int longest = grid.Rows.ToString().Length;
        foreach (var header in grid.RowHeaders) longest = Math.Max(longest, header?.Length ?? 0);
        return Math.Max(CoordHeaderSize, longest * Math.Max(8, 10 * options.Zoom) * 0.62 + 12);
    }
